...
#ifdef __RTTHREAD__
/* GET_INT_SP/FREE_INT_SP: WCH fast-interrupt SP switch (mscratch swap). */
...
{
#ifdef __RTTHREAD__
    /* RT-Thread on Qingke V4: a WCH-Interrupt-fast handler hardware-saves
     * only a partial context. Without the nesting bookkeeping and the
     * interrupt-stack switch, an ISR-time rt_sem_release()/rt_event_send()
     * that wakes a thread would run rt_schedule() on a half-saved context
     * and corrupt the system. Mirrors the BSP drivers (drv_usart_v2.c). */
